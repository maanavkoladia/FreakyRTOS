...
#include "../FreakyRTOS/FIFOsimple.h"
#include "../FreakyRTOS/UART0int.h"
...
sema4_t rx_UART_Wait;
sema4_t tx_UART_Wait;
// Initialize UART0
...
void UART_Init(void){
    ...
    OS_InitSemaphore(&rx_UART_Wait, 0);
    OS_InitSemaphore(&tx_UART_Wait,FIFOSIZE);
}
...
int UART_OutChar(char data){                                        
    //OS_Wait(&tx_UART_Wait);
    while(TxFifo_Put(data) == FIFOFAIL){};                             //------------UART_OutCharNonBlock------------
    UART0_IM_R &= ~UART_IM_TXIM;          // disable TX FIFO interrupt // non blocking output ASCII character to UART
    copySoftwareToHardware();                                          // Input: letter is an 8-bit ASCII character to be transferred
    UART0_IM_R |= UART_IM_TXIM;           // enable TX FIFO interrupt  // Output: none
    return 0;

...
/                   Changed argument of f_chdrive(), f_mkfs(), eDisk_Read() and eDisk_Write().
...
)
{
  ...
  if (eDisk_Status(fil->fs->drv) & STA_NOINIT)
    return FR_NOT_READY;
...
)
{
  ...
  if (eDisk_Write(pdrv, tbl, b_vol, 1) != RES_OK)  /* Write it to the VBR sector */
    return FR_DISK_ERR;
  ...
  for (i = 0; i < N_FATS; i++) {    /* Initialize each FAT copy */
    ...
    if (eDisk_Write(pdrv, tbl, wsect++, 1) != RES_OK)
      return FR_DISK_ERR;
...
)
{
  ...
  /* Write it to the MBR */
  return (eDisk_Write(pdrv, buf, 0, 1) != RES_OK || disk_ioctl(pdrv, CTRL_SYNC, 0) != RES_OK) ? FR_DISK_ERR : FR_OK;
}

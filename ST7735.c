...
sema4_t LCDFree;       // used for mutual exclusion
...
void ST7735_Message(uint32_t  d, uint32_t  l, char *pt, int32_t value){
  OS_Wait(&LCDFree);
  //OS_Wait(&LCDFree);
  //TODO: fix joshs fuck up
  // write this as part of Labs 1 and 2
  //add 8 if on bottom
  if(d){l+=8;}
  if(value == INT32_MAX){
    ST7735_DrawString(0, l, pt, ST7735_WHITE);
    OS_Signal(&LCDFree);
    return;
  }
  ...
  //UART_OutString(outBuf);
	...
	//ST7735_FillRect(0, l*10, 128, 10, ST7735_BLACK);
    //ST7735_OutString(pt);
    //ST7735_OutString(": ");
    //ST7735_OutUDec(value);
  ST7735_DrawString(0, l, outBuf, ST7735_WHITE);
    //ST7735_DrawString(0,l,"Bye\n", ST7735_WHITE);
  OS_Signal(&LCDFree); 
}
